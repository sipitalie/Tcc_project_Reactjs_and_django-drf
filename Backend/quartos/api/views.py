...
class QuartoList(APIView):
    ...
    """
    def get_owner(self, hotel_owner):
        try:
            return Quarto.objects.filter(hotel_owner=hotel_owner)
        except Quarto.DoesNotExist:
            raise Http404

    def get(self, request, format=None): #function to list all bedrooms a hotel
        hotel_owner=request.data['hotel_owner']
        bedrooms=self.get_owner(hotel_owner)
        print(bedrooms)
